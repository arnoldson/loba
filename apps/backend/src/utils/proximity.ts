...
/** How many hours a "like" adds to expires_at */
export const LIKE_TTL_EXTENSION_HOURS = 2;
...
/**
 * Compute the new expires_at after a like, capped at MAX_TTL_HOURS
...
export function computeExtendedExpiry(
  ...
  const maxExpiry = new Date(
    created.getTime() + MAX_TTL_HOURS * 60 * 60 * 1000,
  );

  const extended = new Date(
    current.getTime() + LIKE_TTL_EXTENSION_HOURS * 60 * 60 * 1000,
  );
